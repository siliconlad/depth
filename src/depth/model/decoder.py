...
class Decoder:
    ...
    def __call__(self, x: tuple[Tensor, Tensor, Tensor, Tensor, Tensor]):
        ...
        layer_3_out = self.conv_1_b(
            layer_3_upsampled.cat(layer_3, dim=1).pad((1, 1, 1, 1), mode="reflect")
        ).elu()
        ...
        layer_2_out = self.conv_2_b(
            layer_2_upsampled.cat(layer_2, dim=1).pad((1, 1, 1, 1), mode="reflect")
        ).elu()
        ...
        layer_1_out = self.conv_3_b(
            layer_1_upsampled.cat(layer_1, dim=1).pad((1, 1, 1, 1), mode="reflect")
        ).elu()
        ...
        stem_out = self.conv_4_b(
            stem_upsampled.cat(layer_0, dim=1).pad((1, 1, 1, 1), mode="reflect")
        ).elu()
